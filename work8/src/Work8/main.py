...
def generate_joint_rotation_animation(
    model,
    betas,
    device,
    dtype,
    ...
    max_angle=np.pi / 2,
    num_frames=60,
    out_path="outputs/joint_rotation_animation.gif",
):
    """
    固定 shape，只让指定关节从 0 逐渐旋转到 max_angle，生成 GIF 动画。
    观察该关节权重区域如何被平滑带动。
    """
    ...
    # SMPL body_pose 索引：joint_id 从 1 开始，偏移为 (joint_id - 1) * 3
    pose_start = (joint_id - 1) * 3
    ...
    for i in range(num_frames):
        angle = max_angle * (i / (num_frames - 1))

        body_pose = torch.zeros((1, 23 * 3), dtype=dtype, device=device)
        # 只绕 Z 轴旋转，方便观察弯曲效果
        body_pose[0, pose_start:pose_start + 3] = torch.tensor(
            [0.0, 0.0, angle],
            dtype=dtype,
            device=device,
        )
        ...
        verts = to_numpy(output.vertices[0])
        ...
        draw_mesh(
            ...
            title=f"Joint {joint_id} Rotation: {np.degrees(angle):.1f}°",
        )

        # ===== 修复：新版 matplotlib 用 buffer_rgba() 替代 tostring_rgb() =====
        fig.canvas.draw()
        # 直接获取 RGBA 格式的 numpy 数组，再截取 RGB 三通道
        rgba = np.asarray(fig.canvas.buffer_rgba())
        frame = rgba[:, :, :3]  # 去掉 alpha 通道
        frames.append(frame)
...
def main(args):
    ...
    import inspect
    print("=== smplx.create 函数签名 ===")
    print(inspect.signature(smplx.create))
    print("=== model_dir 实际值 ===")
    print(repr(model_dir))
    print("=== 目录是否存在 ===")
    print(os.path.exists(model_dir))

    from smplx import SMPL

    # model_path 直接指向 smpl 子文件夹（pkl 文件所在的目录）
    model = SMPL(
        model_path=os.path.join(model_dir, "smpl"),
        gender="neutral",
        ext="pkl",
    ...
    generate_joint_rotation_animation(
        model,
        betas,
        device,
        dtype,
        ...
        max_angle=np.pi / 2,
        num_frames=60,
        out_path=os.path.join(out_dir, f"joint_{args.joint_id}_rotation.gif"),
    )
    # ==============================
